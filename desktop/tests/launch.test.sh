...
assert_logged '^foot --app-id wardos-tui-btop -e btop$'
wardos-launch webapp github
assert_logged "^chromium --app=https://github.com --user-data-dir=$XDG_DATA_HOME/wardos/webapps/github --class=wardos-webapp-github$"
...
assert_logged '^foot --app-id wardos-tui-mine -e htop -d 5$'
...
mock hyprctl "case \"\$*\" in 'clients -j') printf '[{\"class\": \"Wardos-TUI-btop\"}]\n' ;; esac"
wardos-launch or-focus wardos-tui-btop wardos-launch tui btop
assert_logged '^hyprctl dispatch focuswindow class:wardos-tui-btop$'
[[ $(grep -c "^foot --app-id wardos-tui-btop" "$MOCK_LOG") -eq 1 ]] || fail "a focused window is not started again"
wardos-launch or-focus wardos-tui-mine wardos-launch tui mine
assert_not_logged '^hyprctl dispatch focuswindow class:wardos-tui-mine$'
[[ $(grep -c "^foot --app-id wardos-tui-mine" "$MOCK_LOG") -eq 2 ]] || fail "or-focus starts a missing window"
exit 0
