...
GitHub remote pinned from the entry snapshot's captured `.git/config` (not re-read
from the live worktree — issue #196) and the entry snapshot's protected paths; `network` is
