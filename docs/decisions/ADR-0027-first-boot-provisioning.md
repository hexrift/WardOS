...
- **Interrupted provisioning recovers via a durable journal + rollback, not idempotent
  ...
  **indeterminate** — the journal is kept and the connection replies *recovery-required* rather
  ...
  reconciled (the orphan removed) on the next boot. Neither a rollback nor a reconcile's own
  journal cleanup ever outruns the account removal it records: `userdel` does not `fsync` its
  own writes to the account database, so the journal — the only recovery record — is cleared
  only *after* that database's own durability is independently confirmed. Confirming durability
  means more than flushing each file's content (`/etc/passwd`, `/etc/shadow`, `/etc/group`,
  `/etc/gshadow`, `/etc/subuid`/`/etc/subgid` where present): `shadow-utils` commits each of
  them by writing a temp file and renaming it into place — the same pattern `write_durable`/
  `remove_durable` use elsewhere — and a rename's directory entry is not durable until the
  containing directory (`/etc`) is itself `fsync`ed, so that runs too, after the files. Until
  all of that succeeds the journal is left completely untouched, never attempted, in *both*
  `rollback_account` (a fresh mid-transaction failure) and `reconcile_pending` (a prior
  transaction found on the next attempt) — so a crash between an account removal and its
  journal record cannot leave a removal whose only record has already been discarded, however
  it was reached. Once that durability is confirmed, a residual failure to clear the journal
  itself is provably harmless (retried once, then reported rather than swallowed) —
  `reconcile_pending` safely tolerates a resurrected journal naming an account already known to
  be durably gone.
...
provisioning would create a second administrator. That reconcile-on-entry is gated the same way
the rest of the unit is (`ConditionPathExists=!/var/lib/wardos/provisioned`): it runs on every
*unprovisioned* boot, but **not** once the marker exists — systemd never even starts `ExecStart`
then. So a **stale journal left behind by its own already-committed marker** (its post-commit
cleanup crashed, or lost the directory `fsync` that would have confirmed it) is *not* actively
reconciled by this unit, or by any other privileged path — there isn't one. It is, instead,
provably **inert**: `wardos-greetd-session` checks the marker *first*, and once it is present any
leftover dev-seed journal cannot be the pre-commit unresolved-transaction state its own guard
exists to catch (a committed marker is the transaction's durable commit point), so the selector
proceeds to the greeter regardless. It makes a best-effort removal attempt on the way there — the
selector runs as the unprivileged `greeter` user against a journal `wardos-dev-seed` created as
root, so this may legitimately fail with EACCES in production, and when it does the failure is
reported honestly (never silently discarded as if it had succeeded) rather than claimed as
"cleaned up" — but nothing downstream depends on that attempt succeeding, and no future boot
comes back to clear it either. The image build asserts no `dev-seed-password`
