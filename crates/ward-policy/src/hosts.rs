...
//!
//! This is the single source of truth for what each [`NetworkCapability`](crate::NetworkCapability)
//! preset means as a set of hosts: the merge lattice ([`crate::merge`]) reasons about
//! egress breadth with it, and `ward-proxy` enforces egress with the very same lists
//! (it re-exports this module), so the decision and the enforcement can never drift.
//!
